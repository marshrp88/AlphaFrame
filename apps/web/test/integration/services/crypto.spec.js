...
vi.mock('tweetnacl', () => {
  ...
  const secretbox = vi.fn((message, nonce, key) => {
  ...
  secretbox.open = vi.fn((box, nonce, key) => {
    if (box && box.length > 16) {
      return box.slice(0, box.length - 16);
    }
    return null;
  });
  secretbox.seal = vi.fn((message, nonce, key) => {
    const mockEncrypted = new Uint8Array(message.length + 16);
    mockEncrypted.set(message);
    mockEncrypted.set(new Uint8Array(16).fill(1), message.length);
    return mockEncrypted;
  ...
  return {
    ...
    default: {
      secretbox,
      hash: vi.fn((message) => new Uint8Array(64).fill(0)),
      randomBytes: vi.fn((length) => new Uint8Array(length).fill(42))
    }
  };
});
...
vi.mock('tweetnacl-util', () => ({
  ...
  default: {
    ...
    decodeBase64: vi.fn((str) => new Uint8Array(Buffer.from(str, 'base64'))),
    encodeUTF8: vi.fn((data) => {
      if (data instanceof Uint8Array) {
        return new TextDecoder().decode(data);
      }
      return 'mock-utf8-encoded';
    }),
    decodeUTF8: vi.fn((str) => new TextEncoder().encode(str))
  }
}));
