...
const box = {
  ...
  seal: jest.fn((message, recipientPublicKey, senderSecretKey) => {
    const mockEncrypted = new Uint8Array(message.length + 16);
    mockEncrypted.set(message);
  ...
  open: jest.fn((box, nonce, recipientSecretKey, senderPublicKey) => {
    if (box && box.length > 16) {
      return box.slice(0, box.length - 16);
    }
    return null;
...
const sign = {
  ...
  sign: jest.fn((message, secretKey) => {
  ...
  signDetached: jest.fn((message, secretKey) => {
    return new Uint8Array(64).fill(1);
  }),
  
  verify: jest.fn((signedMessage, publicKey) => {
  ...
  verifyDetached: jest.fn((message, signature, publicKey) => {
